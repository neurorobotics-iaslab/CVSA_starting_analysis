...
% compute the ERD/ERS not in chanlocs and the fisher score related for each
% sample computed on ERD signals. In addition compute the PSD for all channels
...
for idx_f = 1:length(files)
    ...
    %% Extract trial infoo
    [trialStart, trialStop, fixStart, fixStop, ck, tk] = extract_trial_info(signal, fixPOS, ...
